...
for file in "$TEMP_EXPORT"/*.md; do
  ...
  # This regex matches:
  #  1. A tag starting with '#' but NOT followed by a space (to exclude Markdown headings)
  #  2. A multiword tag wrapped with '#' on both ends
  #  3. Only matches if the tag is not part of a Markdown heading
  tags=$(grep -v '^#' "$file" | grep -oE '(#[[:alnum:]/_-]+)|(#([^#]+)#)' | sort | uniq)
